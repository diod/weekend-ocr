...
def find_qr(bw, minX, maxX, minY, maxY):
  ...
  for x in range( minX, maxX-3*pw-1, 2):
    for y in range( minY + 3*ph, maxY , 2):
      w1  = 2*int(255-bw[y,x]) #black
      w1 += 0.5* (int(255-bw[y+ph,x]) + int(255-bw[y-ph,x]) + int(255-bw[y,x+pw]) + int(255-bw[y,x-pw])) #black

      w1 += 0.5 * (int(bw[y+2*ph, x]) + int(bw[y-2*ph, x]) + int(bw[y,x+2*pw]) + int(bw[y,x-2*pw])); #white
      w1 += 0.5 * (int(bw[y+2*ph, x+2*pw]) + int(bw[y-2*ph, x+2*pw]) + int(bw[y+2*ph,x-2*pw]) + int(bw[y-2*ph,x-2*pw])); #white

      w1 += int(255 - bw[y+3*ph, x+3*pw]) + int(255 - bw[y+3*ph, x-3*pw]) + int(255 - bw[y-3*ph, x+3*pw]) + int(255 - bw[y-3*ph,x-3*pw]); #black
      #white
      if (y-4*ph > 0):
        if (x+4*pw < im_width):
          w1 += int(bw[y-4*ph,x+4*pw])
        else:
          w1 += 255
        w1 += int(bw[y-4*ph,x-4*pw])
      else:
...
img = cv2.medianBlur(bw_img, 5)

#qrimg = cv2.cvtColor(bw_img,cv2.COLOR_BGR2GRAY)
(_,qrimg) = cv2.threshold(img,127,255,cv2.THRESH_BINARY)

##dilate = cv2.Sobel(dilate, cv2.CV_8U, 0,1, ksize=5)
dilate = cv2.dilate(qrimg, cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(3,3)), 1);
dilate = cv2.erode(dilate, cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(3,3)), 1);

#cv2.imshow("edges", edges);

#find QR code
qrcode = get_qrdata(dilate, filename);
print qrcode
